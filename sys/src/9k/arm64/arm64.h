/*
 * arm64-specific definitions for cortex-a72
...
/*
 * MRS opcode is load from coproc; MSR is store to coproc.
 * if op0 == 1, can use SYSL and SYS instructions.
 */
#define MRSN1(sysargs, rt) WORD $(0xd5000000 | (sysargs) | (rt) | 1<<21)
#define MSRN1(rs, sysargs) WORD $(0xd5000000 | (sysargs) | (rs))

/* sorry, can't split SYSARG5 across lines */
...
/* originally from risc-v */
enum {					/* PTE; see mem.h PTE* */
	PteP		= 1<<8,		/* Present/valid */
	PteR		= 1<<7,		/* Read */
	PteW		= 0,		/* read/Write */
	PtenX		= 1LL<<54,	/* no-Execute */
	/* page accessible to U mode; only usable on leaves */
	PteU		= 1<<6,
	PtenG		= 1<<9,		/* not Global; used for lower addresses */

	PteRSW		= 0,		/* reserved for sw use */
	PtePS		= 0,		/* TODO: super page leaf */

	/* leaf pte values for Svpbmt extension in Sv39, Sv48 and Sv57 */
	Ptepma		= 0,		/* none: cacheable */
	Ptenc		= (1LL<<61),	/* non-cacheable memory */
	Pteio		= (2LL<<61),	/* non-cacheable i/o space */

	PteLeaf		= (PteR|PteW|PtenX), /* if 0, next level; non-0 leaf */
	/* some implementations require A & D in leaf ptes to avoid faults */
	Pteleafmand	= 0,		// (PteA|PteD),
