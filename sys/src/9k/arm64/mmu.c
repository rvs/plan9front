/*
 * arm64 memory management for 48-bit virtual addresses.
...
mappage(Page *ptroot, PTE *pte, uintptr va, uintptr pa)
{
	...
	/* executable due to absence of PtenX */
	ptebits = PteR | PteW | PtePS | Pteleafvalid | va2gbit(va);
	if (va < KSEG0)
		ptroot->daddr++;
	if (*pte)
...
/*
 * prepare to start CPUs other than 0.
 *
 ...
 * vsvm page and the Mach page (32K currently).  The layout (Syspercpu) is
...
gohigh(uintptr)
{
	...
	putstvectodo(strap);			/* high virtual */
...
{
	putstvectodo((void *)ensurelow((uintptr)strap)); /* virtual but low */
	if (sys->nonline <= 1)
...
mmuput(uintptr va, uintmem pa, Page*)
{
	...
	ptebits = PTEPPN(pa)|PteU|PteR|attrs|Pteleafvalid;	/* executable */
	if(attrs & PteW)
		ptebits |= PtenX;
	/* avoids faults on 1st use on hardware */
//	ptebits |= PteA | PteD;
...
int
mmuwalknewpte(uintptr va, uintmem mem, int lvl)
{
	int l;
	PTE ptebits;
	PTE *pte;
