...
probemembank(int bankno, Membank *bank)
{
	...
	/*
	 * there's often protected firmware at PHYSMEM or below KTZERO.
	...
	*memstlp = magic;
	coherence();
	if (*memstlp != magic) {
		iprint("start of bank %d (after %#p) is not RAM\n",
			bankno, memst);
		r = -1;
	}
...
mapunmapped2phys(uintptr pa)
{
	...
	for(; sys->vmunmapped < sys->vmend; sys->vmunmapped += PGLSZ(1)){
		 ...
		 */
//		mmuwrpte(pte, PTEPPN(pa)|PteR|PteW|PteX|Pteleafvalid|PteG);
		pa += PGLSZ(1);
	}
	return pa;
}
 ...
 * and after there may be other possibly-discontiguous banks of memory,
 * which are allocated to user processes.
 */
void
meminit(void)
{
