...
#ifndef Lowdebug
#define Lowdebug 0	/* beware: non-0 may throw off MP start-up timing */
#endif
 ...
 * and we can only touch it in machine mode anyway.
 */
static void
pmpinit(void)
{
...
delegate(void)
{
	...
	if (soc.c910) {
		/* these are the priv-1.9 csrs */
		csrswap(MUCOUNTEREN, MASK(3));
		csrswap(MSCOUNTEREN, MASK(3));
	}
...
satpprobe(Sys *lowsys)			/* only safe in machine mode */
{
	...
	if (0) {
		/*
		 * priv 1.9 way: the actual paging mode is mstatus.Vm;
		 * satp is called sptbr.
		 */
		pagingmode = 0;
		putmsts(getmsts() | Sv39_1_9);
	}
	/* probe satp modes */
...
mach_to_super(Sys *lowsys)
{
	...
	putmsts(Defmsts & ~(Mie|Upie|Uie|Tvm|Tw|Tsr|Ube|Mbe64|Sbe64));
	if (getmsts() & Uie)
...
setmtimecmp(int cpu)
{
	...
	if (clint == nil) {
		m->mtimecmp = nil;
		prf("cpu%d: zero clint address\n", cpu);
	} else {
		/* too soon to probe for clint; this breaks on vf2 */
		if (0 && probeulong((ulong *)clint, Read) < 0)
			/* we won't get far without a working clint */
			prf("cpu%d: no response from clint at %#p\n",
				cpu, clint);
		else
			dbprf("cpu%d: clint at %#p\n", cpu, clint);
		m->mtimecmp = &clint->mtimecmp[m->hartid];
	}
}
...
/*
 * on a secondary cpu, secstall waits until stall is released, sets m,
 * waits for go-ahead, returns stack top.
 * another cpu must have allocated a Mach for us previously.
 */
static uintptr
secstall(Sys *lowsys, int cpu)
{
	stagger(cpu);			/* stagger startups for prints */
	dbprf("cpu%d stalling...", cpu);
	/*
	 * wait for cpu0 to allocate secondaries' data structures.
	 * the stall is cleared by schedcpus, settrampargs from reboot.
	 */
	...
	if (cpu == 0)
		dbprf("cpu is 0 in secstall after stall!\n");
	setmach(lowsys, cpu);
...
setsts(void)
{
	...
	putsts((getsts() & ~(Ube|Spp|Uxl|Uie|Mpp|Sbe64)) |
		Spie | Defssts | Mppuser);
}
...
low(uint cpu)
{
	...
	/*
	 * On secondary cpus, wait for cpu0 to clear the stall in main.c.
	 * On all cpus, set m & sys, work out new stack top.
	 */
	stktop = cpu == 0? setstkmach0(lowsys): secstall(lowsys, cpu);
