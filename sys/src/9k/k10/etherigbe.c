 ...
 *	not yet working on i350s on supermicro motherboards
 *
 ...
 * might have better luck.  The 217 code is quarantined and disabled by
 * default; see ALLOW_217.  Windows can drive it with Lenovo's driver, but not
 * with Intel's(!) and the TS140's pxe boot rom can boot through it.  One
 * theory is that Intel's ME is preventing transmission.  Another is that the
 * missing nvram is failing to initialise one of the zillions of registers.
 *
...
enum {
	...
	Nrb	= 384,		/* private receive buffers, ≫ Nrd */
...
enum {					/* [RT]xdctl */
	...
	Qenable		= 0x02000000,	/* Queue Enable (multiq ctlrs) */
...
struct Ctlr {
	...
	uchar	nqueues;	/* 1-8, so far.  not used yet. */
...
i82563tproc(void *v)
{
	...
	for(;;){
		ienable(ctlr, Txdw|Txqe|Txdlow|Parityeccrst);
		/* wake up only to diagnose lack of interrupts */
...
//		r = 0x10000;	/* works for pxe boot rom; manual suggests 0 */
	...
	default:
		if (ctlr->multiq) {
			...
			r = Gran | 1<<WthreshSHIFT | 1 <<PthreshSHIFT |
				1<<HthreshSHIFT;
		break;
	}
	regs[Rxdctl] = r;
...
newctlr(Pcidev *p, int type)
{
	...
	ctlr->multiq = type == i82575 || type == i82576 || type == i82580 ||
		type == i350 || type == i210 || type == i217 || type == i218;
	...
	ctlr->hasfutextnvm = ctlr->type == i217;
