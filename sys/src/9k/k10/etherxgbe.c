...
enum {
	...
	/* tunable parameters */
	/*
	 * File server xmit queue high-water mark is 27.  For receive ring,
	 * mark is 58 and unprocessed buffers mark is 179.
	 ...
	 * Nrb of 256 wasn't enough on fs, but we recovered.
	 */
	Ntd	= 32,		/* multiple of 8 */
	Nrd	= 128,		/* multiple of 8 */
	Nrb	= 512,		/* private receive buffers, >= Nrd */
...
struct Ctlr {
	...
	char	is540;		/* flag: is x540 (slight variant of 599)? */
//	char	is550;		/* flag: is x550? */
...
scanpci(void)
{
	...
	while(p = pcimatch(p, Vintel, 0)){
		...
		case 0x10f4:		/* xf lr */
		case 0x150b:		/* at2 */
			type = I82598;	/* oldest; differs mainly in setup */
			break;
		...
		case 0x1558:		/* x520-qda1 */
			type = I82599;	/* includes x520, second oldest */
			break;
		default:
			continue;
		}
