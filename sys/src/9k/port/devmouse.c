...
{
//	if(!conf.monitor)
//		error("mouseattach with no monitor");
...
mousewalk(Chan *c, Chan *nc, char **name, int nname)
{
	...
	if(wq != nil && wq->clone != c && wq->clone != nil &&
	    (wq->clone->qid.type&QTDIR)==0)
		incref(&mouse);
	return wq;
}
...
{
//	if(!conf.monitor)
//		error("mousecreate with no monitor");
	error(Eperm);
}
...
mousewrite(Chan *c, void *va, long n, vlong)
{
	...
	error("mousewrite to unknown file");
	return -1;
}
