...
static	uchar pgcarved;		/* flag: pgmem carved off a memory bank? */
...
unitsconv(Fmt *f)
{
	...
	switch(f->r) {
	case 'N':
		n = va_arg(f->args, uvlong);
		break;
	default:
		return fmtstrcpy(f, "(unitsconv)");
	}
	prsize(buf, sizeof buf, n);
...
pagesummary(void)
{
	...
	occby = sys->pmoccupied;
#define print iprint		/* TODO: vf debugging */
	if (0 && usedby > occby)
