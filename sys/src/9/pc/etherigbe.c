 ...
 *	adapting to i350:
 *	Intel smashed the 10GbE ivar stuff into the i350,
 *	and the result is messy, with two sets of interrupt registers,
 *	normal and extended.  extended is for ivar queues.
 *
...
enum {
	 ...
	 *
	 * Greatest xmit queue high-water mark is 14.  For receive ring,
	 * mark is 6 and unprocessed buffers mark is 17 on xeons.
...
enum {
	...
	/*
	 * i350 extended interrupt registers for ivar.
...
i82563rxinit(Ctlr* ctlr)
{
	...
	regs = ctlr->regs;
	type = ctlr->type;
	rctl = Dpf | Bsize2048 | Bam | RdtmsHALF;	/* no Vfe, no Secrc */
	/*
	 * The i350 has a bug: it always strips the CRC.
	 *	if (type == i350) rctl |= Secrc;
	 */
